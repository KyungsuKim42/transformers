...
"""Testing suite for the PyTorch TokenSynth model."""
...
def prepare_tokensynth_decoder_inputs_dict(
    config,
    input_ids,
    attention_mask=None,
    head_mask=None,
...
class TokenSynthDecoderTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_tokensynth_decoder_inputs_dict(
            config,
            input_ids,
            encoder_hidden_states=encoder_hidden_states,
        )
    ...
    def get_config(self):
        config = TokenSynthDecoderConfig(
...
@require_torch
class TokenSynthDecoderTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TokenSynthModel, TokenSynthForCausalLM) if is_torch_available() else ()
    greedy_sample_model_classes = (
        (TokenSynthForCausalLM,) if is_torch_available() else ()
    )  # we don't want to run all the generation tests, only a specific subset
    test_pruning = False
    test_resize_embeddings = False

    def setUp(self):
        self.model_tester = TokenSynthDecoderTester(self)
        self.config_tester = ConfigTester(self, config_class=TokenSynthDecoderConfig, hidden_size=16)
    ...
    def check_training_gradient_checkpointing(self, gradient_checkpointing_kwargs=None):
        ...
        model = TokenSynthForCausalLM(config)
        ...
        inputs = self._prepare_for_class(inputs_dict, TokenSynthForCausalLM, return_labels=True)
        ...
        for k, v in model.named_parameters():
            if v.requires_grad:
                self.assertTrue(v.grad is not None, f"{k} in {TokenSynthForCausalLM.__name__} has no gradient!")
    ...
    @unittest.skip(reason="TokenSynth does not use inputs_embeds")
...
def prepare_tokensynth_inputs_dict(
    config,
    input_ids,
    decoder_input_ids,
    attention_mask=None,
...
class TokenSynthTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        config = self.get_config()
        inputs_dict = prepare_tokensynth_inputs_dict(config, input_ids, decoder_input_ids=decoder_input_ids)
        return config, inputs_dict
    ...
    def get_config(self):
        ...
        decoder_config = TokenSynthDecoderConfig(
            vocab_size=self.vocab_size,
            hidden_size=self.hidden_size,
            num_hidden_layers=self.num_hidden_layers,
            num_attention_heads=self.num_attention_heads,
        ...
        config = TokenSynthConfig.from_sub_models_config(text_encoder_config, audio_encoder_config, decoder_config)
        return config
...
@require_torch
class TokenSynthTest(ModelTesterMixin, GenerationTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (TokenSynthForConditionalGeneration,) if is_torch_available() else ()
    greedy_sample_model_classes = (TokenSynthForConditionalGeneration,) if is_torch_available() else ()
    pipeline_model_mapping = {"text-to-audio": TokenSynthForConditionalGeneration} if is_torch_available() else {}
    test_pruning = False  # training is not supported yet for TokenSynth
    ...
    def setUp(self):
        self.model_tester = TokenSynthTester(self)
    ...
    def check_tokensynth_model_output_attentions(
        self,
        model_class,
        config,
        input_ids,
    ...
    def check_tokensynth_model_output_attentions_from_config(
        self,
        model_class,
        config,
        input_ids,
        ...
        **kwargs,
        ...
        # Similar to `check_tokensynth_model_output_attentions`, but with `output_attentions` triggered from the
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            self.check_tokensynth_model_output_attentions(model_class, config, **inputs_dict)
            self.check_tokensynth_model_output_attentions_from_config(model_class, config, **inputs_dict)

    # override since we have a specific forward signature for tokensynth
    ...
    # override since the `input_ids` cannot be used as the `decoder_input_ids` for tokensynth (input / outputs are
    ...
    @unittest.skip("TokenSynthModel is actually not the base of TokenSynthForCausalLM as the latter is a composit model")
...
@require_torch
class TokenSynthIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return TokenSynthForConditionalGeneration.from_pretrained("KyungsuKim/TokenSynth").to(torch_device)

    @cached_property
    def processor(self):
        return TokenSynthProcessor.from_pretrained("KyungsuKim/TokenSynth")
...
@require_torch
class TokenSynthStereoIntegrationTests(unittest.TestCase):
    @cached_property
    def model(self):
        return TokenSynthForConditionalGeneration.from_pretrained("facebook/tokensynth-stereo-small").to(torch_device)

    @cached_property
    def processor(self):
        return TokenSynthProcessor.from_pretrained("facebook/tokensynth-stereo-small")
