...
from .configuration_tokensynth import TokenSynthConfig, TokenSynthDecoderConfig
...
_CONFIG_FOR_DOC = "TokenSynthConfig"
_CHECKPOINT_FOR_DOC = "KyungsuKim/TokenSynth"
...
@dataclass
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenUnconditionalInput with Musicgen->TokenSynth
class TokenSynthUnconditionalInput(ModelOutput):
    """
    Args:
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenSinusoidalPositionalEmbedding with Musicgen->TokenSynth
class TokenSynthSinusoidalPositionalEmbedding(nn.Module):
...
# Copied from transformers.models.bart.modeling_bart.BartAttention with Bart->TokenSynth
class TokenSynthAttention(nn.Module):
    ...
    def __init__(
        ...
        config: Optional[TokenSynthConfig] = None,
    ):
        super().__init__()
...
# Copied from transformers.models.bart.modeling_bart.BartFlashAttention2 with Bart->TokenSynth
class TokenSynthFlashAttention2(TokenSynthAttention):
    """
    TokenSynth flash attention module. This module inherits from `TokenSynthAttention` as the weights of the module stays
    ...
    def forward(
        ...
        # TokenSynthFlashAttention2 attention does not support output_attentions
        if output_attentions:
            raise ValueError("TokenSynthFlashAttention2 attention does not support output_attentions")
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenSdpaAttention with Musicgen->TokenSynth
class TokenSynthSdpaAttention(TokenSynthAttention):
    def forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "TokenSynthModel is using TokenSynthSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
...
TOKENSYNTH_ATTENTION_CLASSES = {
    "eager": TokenSynthAttention,
    "sdpa": TokenSynthSdpaAttention,
    "flash_attention_2": TokenSynthFlashAttention2,
}


# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenDecoderLayer with MUSICGEN->TOKENSYNTH,Musicgen->TokenSynth
class TokenSynthDecoderLayer(nn.Module):
    def __init__(self, config: TokenSynthDecoderConfig):
        super().__init__()
        self.embed_dim = config.hidden_size

        self.self_attn = TOKENSYNTH_ATTENTION_CLASSES[config._attn_implementation](
        ...
        self.encoder_attn = TOKENSYNTH_ATTENTION_CLASSES[config._attn_implementation](
            self.embed_dim,
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenPreTrainedModel with Musicgen->TokenSynth
class TokenSynthPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = TokenSynthDecoderConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["TokenSynthDecoderLayer", "TokenSynthAttention"]
...
TOKENSYNTH_START_DOCSTRING = r"""

    The TokenSynth model was proposed in [Simple and Controllable Music Generation](https://arxiv.org/abs/2306.05284) by
    ...
    Parameters:
        config ([`TokenSynthConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""

TOKENSYNTH_INPUTS_DOCSTRING = r"""
    Args:
...
TOKENSYNTH_DECODER_INPUTS_DOCSTRING = r"""
    Args:
...
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenDecoder with MUSICGEN->TOKENSYNTH,Musicgen->TokenSynth
class TokenSynthDecoder(TokenSynthPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`TokenSynthDecoderLayer`]
    """
    ...
    def __init__(self, config: TokenSynthDecoderConfig):
        ...
        self.embed_positions = TokenSynthSinusoidalPositionalEmbedding(
            config.max_position_embeddings,
            config.hidden_size,
        )

        self.layers = nn.ModuleList([TokenSynthDecoderLayer(config) for _ in range(config.num_hidden_layers)])
    ...
    @add_start_docstrings_to_model_forward(TOKENSYNTH_DECODER_INPUTS_DOCSTRING)
    def forward(
        self,
...
@add_start_docstrings(
    "The bare TokenSynth decoder model outputting raw hidden-states without any specific head on top.",
    TOKENSYNTH_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenModel with MUSICGEN->TOKENSYNTH,Musicgen->TokenSynth
class TokenSynthModel(TokenSynthPreTrainedModel):
    def __init__(self, config: TokenSynthDecoderConfig):
        super().__init__(config)
        self.decoder = TokenSynthDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
...
@add_start_docstrings(
    "The TokenSynth decoder model with a language modelling head on top.",
    TOKENSYNTH_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenForCausalLM with MUSICGEN->TOKENSYNTH,Musicgen->TokenSynth,musicgen->tokensynth,MusicGen->TokenSynth
class TokenSynthForCausalLM(TokenSynthPreTrainedModel):
    def __init__(self, config: TokenSynthDecoderConfig):
        super().__init__(config)

        self.model = TokenSynthModel(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # per codebook cross-entropy
            # ref: https://github.com/facebookresearch/audiocraft/blob/69fea8b290ad1b4b40d28f92d1dfc0ab01dbab85/audiocraft/solvers/tokensynth.py#L242-L243
    ...
    def generate(
        ...
        **kwargs,
        ...
        # Build the delay pattern mask for offsetting each codebook prediction by 1 (this behaviour is specific to TokenSynth)
        input_ids, delay_pattern_mask = self.build_delay_pattern_mask(
            input_ids,
...
@add_start_docstrings(
    "The composite TokenSynth model with a text encoder, audio encoder and TokenSynth decoder, "
    "for music generation tasks with one or both of text and audio prompts.",
    TOKENSYNTH_START_DOCSTRING,
)
# Copied from transformers.models.musicgen.modeling_musicgen.MusicgenForConditionalGeneration with MUSICGEN->TOKENSYNTH,Musicgen->TokenSynth,musicgen->tokensynth,MusicGen->TokenSynth,facebook/musicgen-small->KyungsuKim/TokenSynth
class TokenSynthForConditionalGeneration(PreTrainedModel):
    config_class = TokenSynthConfig
    ...
    def __init__(
        ...
        decoder: Optional[TokenSynthForCausalLM] = None,
    ):
        if config is None and (text_encoder is None or audio_encoder is None or decoder is None):
            raise ValueError(
                "Either a configuration has to be provided, or all three of text encoder, audio encoder and TokenSynth decoder."
            )
        if config is None:
            config = TokenSynthConfig.from_sub_models_config(text_encoder.config, audio_encoder.config, decoder.config)
        else:
        ...
        if config.decoder.cross_attention_hidden_size is not None:
            if config.decoder.cross_attention_hidden_size != config.text_encoder.hidden_size:
                raise ValueError(
                    "If `cross_attention_hidden_size` is specified in the TokenSynth decoder's configuration, it has to be equal"
        ...
        if decoder is None:
            decoder = TokenSynthForCausalLM(config.decoder)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        ```python
        >>> from transformers import TokenSynthForConditionalGeneration

        >>> model = TokenSynthForConditionalGeneration.from_pretrained("KyungsuKim/TokenSynth")
        ```"""

        # At the moment fast initialization is not supported for composite models
        if kwargs.get("_fast_init", False):
            logger.warning(
                "Fast initialization is currently not supported for TokenSynthForConditionalGeneration. "
    ...
    def from_sub_models_pretrained(
        ...
        **kwargs,
        ...
        r"""
        Instantiate a text encoder, an audio encoder, and a TokenSynth decoder from one, two or three base classes of the
        ...
        >>> # initialize a tokensynth model from a t5 text encoder, encodec audio encoder, and tokensynth decoder
        >>> model = TokenSynthForConditionalGeneration.from_sub_models_pretrained(
        ...     text_encoder_pretrained_model_name_or_path="google-t5/t5-base",
        ...     audio_encoder_pretrained_model_name_or_path="facebook/encodec_24khz",
        ...     decoder_pretrained_model_name_or_path="KyungsuKim/TokenSynth",
        ... )
        >>> # saving model after fine-tuning
        >>> model.save_pretrained("./tokensynth-ft")
        >>> # load fine-tuned model
        >>> model = TokenSynthForConditionalGeneration.from_pretrained("./tokensynth-ft")
        ```"""
        ...
        if decoder is None:
            ...
            if "config" not in kwargs_decoder:
                ...
                if isinstance(decoder_config, TokenSynthConfig):
            ...
            decoder = TokenSynthForCausalLM.from_pretrained(decoder_pretrained_model_name_or_path, **kwargs_decoder)

        # instantiate config with corresponding kwargs
        config = TokenSynthConfig.from_sub_models_config(
            text_encoder.config, audio_encoder.config, decoder.config, **kwargs
        )
        return cls(text_encoder=text_encoder, audio_encoder=audio_encoder, decoder=decoder, config=config)

    @add_start_docstrings_to_model_forward(TOKENSYNTH_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=Seq2SeqLMOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        ...
        **kwargs,
        ...
        Examples:
        ```python
        >>> from transformers import AutoProcessor, TokenSynthForConditionalGeneration
        >>> import torch

        >>> processor = AutoProcessor.from_pretrained("KyungsuKim/TokenSynth")
        >>> model = TokenSynthForConditionalGeneration.from_pretrained("KyungsuKim/TokenSynth")
    ...
    def get_unconditional_inputs(self, num_samples=1):
        ...
        return TokenSynthUnconditionalInput(
