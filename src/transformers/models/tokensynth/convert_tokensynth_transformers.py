...
"""Convert TokenSynth checkpoints from the original repository."""
...
import torch
from audiocraft.models import TokenSynth
...
from transformers.models.tokensynth.modeling_tokensynth import TokenSynthForCausalLM
...
def rename_state_dict(state_dict: OrderedDict, hidden_size: int) -> Tuple[Dict, Dict]:
    """Function that takes the fairseq TokenSynth state dict and renames it according to the HF
...
def decoder_config_from_checkpoint(checkpoint: str) -> TokenSynthDecoderConfig:
    if checkpoint.endswith("small"):
        ...
        raise ValueError(
            ...
            "`['facebook/tokensynth-stereo-small', 'facebook/tokensynth-stereo-medium', 'facebook/tokensynth-stereo-large']` "
...
@torch.no_grad()
def convert_tokensynth_checkpoint(
    checkpoint, pytorch_dump_folder=None, repo_id=None, device="cpu", safe_serialization=False
):
    fairseq_model = TokenSynth.get_pretrained(checkpoint, device=device)
    ...
    decoder = TokenSynthForCausalLM(decoder_config).eval()
    ...
    model = TokenSynthForConditionalGeneration(text_encoder=text_encoder, audio_encoder=audio_encoder, decoder=decoder)
    ...
    processor = TokenSynthProcessor(feature_extractor=feature_extractor, tokenizer=tokenizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="small",
        type=str,
        help="Checkpoint size of the TokenSynth model you'd like to convert. Can be one of: "
    ...
    convert_tokensynth_checkpoint(args.checkpoint, args.pytorch_dump_folder, args.push_to_hub)
